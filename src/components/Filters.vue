<template>
  <div class="filters row my-0">
    <div class="col-12 col-lg-6">
      <div class="row buttons">
        <div class="col-6 col-lg-12 px-0 align-self-lg-end">
          <button-filter v-on:clicked="toggleButton" :filter="currency"/>
        </div>
        <div class="col-6 col-lg-12 px-0">
          <button-filter v-on:clicked="toggleButton" :filter="familyRisk"/>
        </div>
      </div>
    </div>
    <div class="col-12 col-lg-6 selector">
      <div class="row">
        <div class="col-6 col-lg-12 px-0 align-self-lg-end">
          <p>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M2.7024 9.6C2.50099 10.3841 2.39938 11.1905 2.4 12C2.4 12.828 2.5056 13.632 2.7024 14.4H6.12C5.95822 12.8041 5.95822 11.1959 6.12 9.6H2.7024ZM3.684 7.2H6.4992C6.8184 5.7396 7.2792 4.4316 7.8444 3.3432C6.10222 4.1825 4.65266 5.52628 3.684 7.2ZM21.2976 9.6H17.88C18.0418 11.1959 18.0418 12.8041 17.88 14.4H21.2976C21.4944 13.632 21.6 12.828 21.6 12C21.6 11.172 21.4944 10.368 21.2976 9.6ZM20.316 7.2C19.3473 5.52628 17.8978 4.1825 16.1556 3.3432C16.722 4.4316 17.1816 5.7396 17.5008 7.2H20.316ZM8.5344 9.6C8.44449 10.3968 8.39962 11.1981 8.4 12C8.4 12.822 8.4456 13.626 8.5344 14.4H15.4656C15.6463 12.8051 15.6463 11.1949 15.4656 9.6H8.5344ZM8.964 7.2H15.036C14.8176 6.29796 14.5026 5.42209 14.0964 4.5876C13.3428 3.0816 12.5364 2.4 12 2.4C11.4624 2.4 10.6572 3.0816 9.9036 4.5876C9.5256 5.346 9.2076 6.228 8.964 7.2ZM3.684 16.8C4.65266 18.4737 6.10222 19.8175 7.8444 20.6568C7.278 19.5684 6.8184 18.2604 6.4992 16.8H3.684ZM20.316 16.8H17.5008C17.1816 18.2604 16.7208 19.5684 16.1556 20.6568C17.8978 19.8175 19.3473 18.4737 20.316 16.8ZM8.964 16.8C9.2076 17.772 9.5256 18.654 9.9036 19.4124C10.6572 20.9184 11.4636 21.6 12 21.6C12.5376 21.6 13.3428 20.9184 14.0964 19.4124C14.4744 18.654 14.7924 17.772 15.036 16.8H8.964ZM12 24C5.3724 24 0 18.6276 0 12C0 5.3724 5.3724 0 12 0C18.6276 0 24 5.3724 24 12C24 18.6276 18.6276 24 12 24Z"
                fill="#BDBDBD"
              />
            </svg>
            ALL
          </p>
          <p>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <circle cx="12" cy="12" r="12" fill="#26234E" />
              <path
                d="M12 2L12.2245 2.69098H12.9511L12.3633 3.11803L12.5878 3.80902L12 3.38197L11.4122 3.80902L11.6367 3.11803L11.0489 2.69098H11.7755L12 2Z"
                fill="#FBB33B"
              />
              <path
                d="M4 11L4.22451 11.691H4.95106L4.36327 12.118L4.58779 12.809L4 12.382L3.41221 12.809L3.63673 12.118L3.04894 11.691H3.77549L4 11Z"
                fill="#FBB33B"
              />
              <path
                d="M5 7L5.22451 7.69098H5.95106L5.36327 8.11803L5.58779 8.80902L5 8.38197L4.41221 8.80902L4.63673 8.11803L4.04894 7.69098H4.77549L5 7Z"
                fill="#FBB33B"
              />
              <path
                d="M5 15L5.22451 15.691H5.95106L5.36327 16.118L5.58779 16.809L5 16.382L4.41221 16.809L4.63673 16.118L4.04894 15.691H4.77549L5 15Z"
                fill="#FBB33B"
              />
              <path
                d="M19 7L19.2245 7.69098H19.9511L19.3633 8.11803L19.5878 8.80902L19 8.38197L18.4122 8.80902L18.6367 8.11803L18.0489 7.69098H18.7755L19 7Z"
                fill="#FBB33B"
              />
              <path
                d="M19 15L19.2245 15.691H19.9511L19.3633 16.118L19.5878 16.809L19 16.382L18.4122 16.809L18.6367 16.118L18.0489 15.691H18.7755L19 15Z"
                fill="#FBB33B"
              />
              <path
                d="M16 18L16.2245 18.691H16.9511L16.3633 19.118L16.5878 19.809L16 19.382L15.4122 19.809L15.6367 19.118L15.0489 18.691H15.7755L16 18Z"
                fill="#FBB33B"
              />
              <path
                d="M8 4L8.22451 4.69098H8.95106L8.36327 5.11803L8.58779 5.80902L8 5.38197L7.41221 5.80902L7.63673 5.11803L7.04894 4.69098H7.77549L8 4Z"
                fill="#FBB33B"
              />
              <path
                d="M8 18L8.22451 18.691H8.95106L8.36327 19.118L8.58779 19.809L8 19.382L7.41221 19.809L7.63673 19.118L7.04894 18.691H7.77549L8 18Z"
                fill="#FBB33B"
              />
              <path
                d="M16 4L16.2245 4.69098H16.9511L16.3633 5.11803L16.5878 5.80902L16 5.38197L15.4122 5.80902L15.6367 5.11803L15.0489 4.69098H15.7755L16 4Z"
                fill="#FBB33B"
              />
              <path
                d="M19.9999 11L20.2244 11.691H20.9509L20.3632 12.118L20.5877 12.809L19.9999 12.382L19.4121 12.809L19.6366 12.118L19.0488 11.691H19.7754L19.9999 11Z"
                fill="#FBB33B"
              />
              <path
                d="M12 20L12.2245 20.691H12.9511L12.3633 21.118L12.5878 21.809L12 21.382L11.4122 21.809L11.6367 21.118L11.0489 20.691H11.7755L12 20Z"
                fill="#FBB33B"
              />
            </svg>
            EURO
          </p>
        </div>
        <div class="col-6 col-lg-12 px-0">
          <p>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <circle cx="12" cy="12" r="12" fill="white" />
              <circle cx="12" cy="12" r="4" fill="#FF003D" />
            </svg>
            JPY
          </p>
          <p>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <circle cx="12" cy="12" r="12" fill="white" />
              <mask
                id="mask0"
                mask-type="alpha"
                maskUnits="userSpaceOnUse"
                x="0"
                y="0"
                width="24"
                height="24"
              >
                <circle cx="12" cy="12" r="12" fill="white" />
              </mask>
              <g mask="url(#mask0)">
                <rect x="1" width="22" height="4" fill="#FF003D" />
                <rect x="1" y="7" width="24" height="4" fill="#FF003D" />
                <rect x="-1" y="14" width="26" height="4" fill="#FF003D" />
                <rect x="1" y="21" width="22" height="4" fill="#FF003D" />
                <rect x="-6" y="-1" width="18" height="12" fill="#17163A" />
              </g>
            </svg>
            USD
          </p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import ButtonFilter from "@/components/ButtonFilter.vue";

export default {
  name: "filters",
  components: {
    ButtonFilter
  },
  data() {
    return {
      currency: {
        name: "Currency",
        type: "ALL",
        pathD:
          "M35 17V15C34.2044 15 33.4413 14.6839 32.8787 14.1213C32.3161 13.5587 32 12.7956 32 12C32 11.2044 32.3161 10.4413 32.8787 9.87868C33.4413 9.31607 34.2044 9 35 9V8C35 7.73478 35.1054 7.48043 35.2929 7.29289C35.4804 7.10536 35.7348 7 36 7C36.2652 7 36.5196 7.10536 36.7071 7.29289C36.8946 7.48043 37 7.73478 37 8V9H37.022C37.8118 9 38.5693 9.31375 39.1278 9.87224C39.6862 10.4307 40 11.1882 40 11.978C40 12.2432 39.8946 12.4976 39.7071 12.6851C39.5196 12.8726 39.2652 12.978 39 12.978C38.7348 12.978 38.4804 12.8726 38.2929 12.6851C38.1054 12.4976 38 12.2432 38 11.978C38 11.7186 37.897 11.4699 37.7136 11.2864C37.5301 11.103 37.2814 11 37.022 11H37V13C37.7956 13 38.5587 13.3161 39.1213 13.8787C39.6839 14.4413 40 15.2044 40 16C40 16.7956 39.6839 17.5587 39.1213 18.1213C38.5587 18.6839 37.7956 19 37 19V20C37 20.2652 36.8946 20.5196 36.7071 20.7071C36.5196 20.8946 36.2652 21 36 21C35.7348 21 35.4804 20.8946 35.2929 20.7071C35.1054 20.5196 35 20.2652 35 20V19H34.949C34.1669 19 33.4168 18.6893 32.8637 18.1363C32.3107 17.5832 32 16.8331 32 16.051C32 15.7858 32.1054 15.5314 32.2929 15.3439C32.4804 15.1564 32.7348 15.051 33 15.051C33.2652 15.051 33.5196 15.1564 33.7071 15.3439C33.8946 15.5314 34 15.7858 34 16.051C34.0003 16.3026 34.1003 16.5438 34.2782 16.7218C34.4562 16.8997 34.6974 16.9997 34.949 17H35ZM37 17C37.2652 17 37.5196 16.8946 37.7071 16.7071C37.8946 16.5196 38 16.2652 38 16C38 15.7348 37.8946 15.4804 37.7071 15.2929C37.5196 15.1054 37.2652 15 37 15V17ZM35 11C34.7348 11 34.4804 11.1054 34.2929 11.2929C34.1054 11.4804 34 11.7348 34 12C34 12.2652 34.1054 12.5196 34.2929 12.7071C34.4804 12.8946 34.7348 13 35 13V11ZM36 24C30.477 24 26 19.523 26 14C26 8.477 30.477 4 36 4C41.523 4 46 8.477 46 14C46 19.523 41.523 24 36 24ZM36 22C38.1217 22 40.1566 21.1571 41.6569 19.6569C43.1571 18.1566 44 16.1217 44 14C44 11.8783 43.1571 9.84344 41.6569 8.34315C40.1566 6.84285 38.1217 6 36 6C33.8783 6 31.8434 6.84285 30.3431 8.34315C28.8429 9.84344 28 11.8783 28 14C28 16.1217 28.8429 18.1566 30.3431 19.6569C31.8434 21.1571 33.8783 22 36 22Z",
        clicked: true
      },
      familyRisk: {
        name: "Family Risk",
        type: "ALL",
        pathD:
          "M26.832 19C26.2814 17.7384 25.9981 16.3765 26 15C26 9.477 30.477 5 36 5C41.523 5 46 9.477 46 15C46.0019 16.3765 45.7186 17.7384 45.168 19H26.832ZM43.748 17C44.0515 15.8183 44.0807 14.5828 43.8334 13.388C43.586 12.1933 43.0687 11.0709 42.3209 10.1068C41.5732 9.14276 40.6148 8.36248 39.5192 7.82569C38.4235 7.2889 37.2196 7.00984 35.9995 7.00984C34.7794 7.00984 33.5755 7.2889 32.4798 7.82569C31.3842 8.36248 30.4258 9.14276 29.6781 10.1068C28.9303 11.0709 28.413 12.1933 28.1656 13.388C27.9183 14.5828 27.9475 15.8183 28.251 17H43.748ZM36 10C35.7348 10 35.4804 9.89464 35.2929 9.70711C35.1054 9.51957 35 9.26522 35 9C35 8.73478 35.1054 8.48043 35.2929 8.29289C35.4804 8.10536 35.7348 8 36 8C36.2652 8 36.5196 8.10536 36.7071 8.29289C36.8946 8.48043 37 8.73478 37 9C37 9.26522 36.8946 9.51957 36.7071 9.70711C36.5196 9.89464 36.2652 10 36 10ZM31 14C30.7348 14 30.4804 13.8946 30.2929 13.7071C30.1054 13.5196 30 13.2652 30 13C30 12.7348 30.1054 12.4804 30.2929 12.2929C30.4804 12.1054 30.7348 12 31 12C31.2652 12 31.5196 12.1054 31.7071 12.2929C31.8946 12.4804 32 12.7348 32 13C32 13.2652 31.8946 13.5196 31.7071 13.7071C31.5196 13.8946 31.2652 14 31 14ZM41 14C40.7348 14 40.4804 13.8946 40.2929 13.7071C40.1054 13.5196 40 13.2652 40 13C40 12.7348 40.1054 12.4804 40.2929 12.2929C40.4804 12.1054 40.7348 12 41 12C41.2652 12 41.5196 12.1054 41.7071 12.2929C41.8946 12.4804 42 12.7348 42 13C42 13.2652 41.8946 13.5196 41.7071 13.7071C41.5196 13.8946 41.2652 14 41 14ZM36 11C36.2652 11 36.5196 11.1054 36.7071 11.2929C36.8946 11.4804 37 11.7348 37 12V15C37 15.2652 36.8946 15.5196 36.7071 15.7071C36.5196 15.8946 36.2652 16 36 16C35.7348 16 35.4804 15.8946 35.2929 15.7071C35.1054 15.5196 35 15.2652 35 15V12C35 11.7348 35.1054 11.4804 35.2929 11.2929C35.4804 11.1054 35.7348 11 36 11Z",
         clicked: false
      }
    };
  },
  methods: {
      toggleButton(event){
          if(event === "Currency"){
              this.currency.clicked = true;
              this.familyRisk.clicked = false;
          } else {
              this.currency.clicked = false;
              this.familyRisk.clicked = true;
          }
      }
  }
};
</script>

<style lang="scss">
$breakpoint-desktop: 992px;

.filters {
  margin-top: 75px;
  @media (min-width: $breakpoint-desktop) {
    margin-top: 0;
  }
}

.filters,
.col-12,
.row {
  @media (min-width: $breakpoint-desktop) {
    height: 100%;
  }
}

.row.buttons {
  margin-top: 60px;
  @media (min-width: $breakpoint-desktop) {
    margin-top: 0;
    top: 0;
  }
}

span {
  font-size: 12px;
  line-height: 16px;
}

.selector {
  @media (min-width: $breakpoint-desktop) {
    background-color: #f2f2f2;
  }
}
</style>
